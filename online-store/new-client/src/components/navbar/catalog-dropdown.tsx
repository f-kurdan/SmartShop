        ...
        <div className="opacity-100 ralative group inline-block w-40 text-center py-[9px]">
            ...
            <div className="shadow-md absolute top-12 w-40 bg-white hidden z-10 group-hover:block">
                ...
                : categories?.length ? categories?.map((cat) =>
                    (<Link key={cat.id} className='py-1 hover:bg-gradient-to-br hover:text-white hover:from-indigo-500 hover:via-sky-600 hover:to-blue-700 block active:from-purple-950 active:via-red-700 active:to-yellow-600' href={createURL(cat.id)}>{cat.name.toUpperCase()}</Link>)
                ) : (
