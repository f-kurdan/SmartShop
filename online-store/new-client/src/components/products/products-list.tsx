...
const ProductsList = ({ products }: { products: productsList }) => {
    ...
    return (
        <div className='flex flex-row gap-2 flex-wrap justify-start items-center'>
            ...
            (<Link className='' href={createURL(product.model, product.color, product.storage)}>
                <div key={product.id} className=' transition-all duration-300 flex flex-col gap-2 justify-evenly items-center p-5 bg-white hover:shadow-lg hover:shadow-black/30 cursor-pointer active:opacity-80 w-72 h-80'>
