      ...
      {categories.map((cat) => (
        <div key={cat.id} className='relative group max-w-[300px] max-h-[300px] rounded-3xl overflow-hidden cursor-pointer '>
          <Link href={`${createURL(cat.id)}`}>
            ...
            <div className='absolute transition duration-500 text-2xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-center opacity-5 group-hover:opacity-100' >{cat.name}</div>
          </Link>
        </div>
      ))}
    </div>
